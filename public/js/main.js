$(function (){
    ...
    $(".tours_btn_update").on('click',function (){
        const title = $(this).parent().parent().children("td:nth-child(2)").text();
        const content = $(this).parent().parent().children("td:nth-child(3)").text();
        const price = $(this).parent().parent().children("td:nth-child(4)").text();
        const imgUrl = $(this).parent().parent().children("td:nth-child(5)").children("img").attr("src");
        const actionUrl = $(this).data("id");
        ...
        $(".news_update_title input").val(title);
        $(".news_update_content textarea").val(content);
        $(".news_update_price textarea").val(price);
        $(".news_update_img img").attr("src",imgUrl);
        $(".news_update_form").attr("action","/cms_tours/"+actionUrl+"/update")
    ...
    $(".tours_btn_create").on('click',function () {
        $(".news_update_form").attr("action","/cms_tours");
        $(".news_update_img img").hide();
        $('.cms_table').hide();
        $('.cms_edit').show();
    })
